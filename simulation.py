...
class Simulation:
    def __init__(self, screen_width, screen_height, disease: Disease):
        ...
        # self.air_travel_lines = [] # Removed as it's for drawing

        # self.num_cores = min(multiprocessing.cpu_count(), 10)  # Removed multiprocessing
    ...
    def update(self):
        ...
        self.susceptible_grid, self.exposed_grid, self.infected_grid, self.recovered_grid, self.dead_grid = \
            update_chunk((
                self.susceptible_grid,
                self.exposed_grid,
                self.infected_grid,
                self.recovered_grid,
                self.dead_grid,
        ...
        # Handle long-distance transmission (single-threaded)
            ...
            if self.total_infected > 1000 and random.random() < self.disease.transmission_air * 0.05:
                ...
                if len(infected_cities) > 0 and len(high_pop_cities) > 0:
                    ...
                    if self.susceptible_grid[target_y, target_x] > 100:
                        ...
                        # self.air_travel_lines.append(((source_x, source_y), (target_x, target_y))) # Removed as it's for drawing
